...
const ParkingRates = () => {
  ...
  return (
    <div className="min-h-screen bg-[#e9dbcf] p-6 flex flex-col items-center">
        ...
        {parkingOptions.map((option, idx) => (
          <div
            ...
            className={`flex justify-between items-center border-2 rounded-lg p-4 cursor-pointer transition-all ${
              selectedRate?.rate === option.rate ? 'border-yellow-400 bg-white' : 'border-black bg-[#e7dcd1]'
            }`}
          >
      ...
      <button
        ...
        className="mt-6 px-6 py-3 bg-gradient-to-r from-lime-400 to-black text-white font-semibold text-lg rounded shadow-md hover:scale-105 transition-transform"
      >
        Generate Receipt
      </button>
...
//     const doc = new jsPDF();
//     doc.setFontSize(16);
//     doc.text('🅿️ Parking Receipt', 20, 20);
//     doc.setFontSize(12);
//     doc.text(`Vehicle Number: ${data.vehicleNumber}`, 20, 40);
//     doc.text(`Rate: ₹${data.amount} (${data.duration})`, 20, 50);
//     doc.text(`Date/Time: ${data.timestamp}`, 20, 60);

//     doc.save(`Receipt_${data.vehicleNumber}_${Date.now()}.pdf`);
//   };
...
//       {receipt && (
//         <div className="mt-10 p-4 bg-white rounded-lg shadow-lg w-full max-w-md border">
//           <h2 className="text-xl font-bold mb-2 text-green-700">Receipt Preview</h2>
//           <p><strong>Vehicle:</strong> {receipt.vehicleNumber}</p>
//           <p><strong>Rate:</strong> ₹{receipt.amount} ({receipt.duration})</p>
//           <p><strong>Date/Time:</strong> {receipt.timestamp}</p>
//         </div>
